...
def draw_view(json_path, keys):
    args = parse_args()
    # 下面为可修改参数
    args.__dict__['task'] = 'plot_curve'
    # print(type(args.__dict__['json_logs']))
    args.__dict__['json_logs'] = json_path
    args.__dict__['keys'] = keys
    args.__dict__['start_epoch'] = '1'
    args.__dict__['eval_interval'] = '1'
    args.__dict__['title'] = None
    args.__dict__['legend'] = None
    args.__dict__['backend'] = None
    args.__dict__['style'] = 'dark'
    args.__dict__['out'] = './result/log'
    ...
    return './result/log.png'


json_path = ["json\log.json"]
keys = ["loss_mask","loss_cls","loss_bbox"]

draw_view(json_path, keys)
